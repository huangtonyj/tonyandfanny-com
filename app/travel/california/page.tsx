"use client";

import { useState, Suspense } from "react";
import { motion, AnimatePresence } from "framer-motion";

export default function California() {
  return "yo, there is going to be some travel tips coming";
}

/*
Northern California
  Redwood National Park
  Burney Falls
  Lassen Volcanic National Park
  Lake Tahoe
  San Francisco
  Napa + Sonoma
  
California Sierra Nevada
  Yosemite National Park
  Sequoia National Park
  Kings Canyon National Park
  Death Valley National Park
  Eastern Sierra (Bishop, Lone Pine, Mammoth Lakes, June Lake, Lee Vining, Bridgeport, ...)
  Mammoth Lakes

Central California
  Pacific Coast Highway (San Francisco to Los Angeles)
  Moneterrey
  Santa Cruz
  Big Sur
  Pismo Beach
  Santa Barbara
  Los Angeles
  
Southern California
  Los Angeles
    Santa Monica
    Griffith Observatory
    Hollywood
    Culver City
    San Gabriel Valley (Monterey Park, Alhambra, Pasadena, Rosemead, Temple City, Arcadia, ...)
  Orange County
    OC & Lau
    Disneyland
  San Diego
  Joshua Tree National Park

Beyond California
  Las Vegas
  Vancouver
  Seattle
  Portland
  Columbia River Gorge
  Mt. Rainier
  Olympic National Park
  Cascades National Park
  Crater Lake
  Cannon Beach
  Pacific Coast Highway (Seattle to San Francisco)
  Zion National Park
  Bryce Canyon National Park
  Grand Canyon National Park
  Antelope Canyon
  Horseshoe Bend
  Arches National Park
  Canyonlands National Park
  Monument Valley
  Mesa Verde National Park
*/
